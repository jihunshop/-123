...
async def on_message(message):
    ...
    if (message.author.id == 776405291838865418):
    ...
    if not isinstance(message.channel, discord.channel.DMChannel):
        if (message.author.id == message.guild.owner_id or message.author.id == 776405291838865418):
                    ...
                    if (message.content == "!충전"):
                        ...
                        if (server_info[3] != "" and server_info[4] != ""):
                            if (user_info != None):
                                ...
                                try:
                                    jsondata = {"token" : "IvKHj5TB8WqQBdJaoE0aWrRtbQ4Cha", "id" : server_info[3], "pw" : server_info[4], "pin" : msg.content}
                                    res = requests.post("http://xoker.tk/api", json=jsondata)
                                    ...
                                    if (len(product_info[2].split("\n")) >= buy_amount):
                                        if (int(user_info[1]) >= int(product_info[1] * buy_amount)):
                                            ...
                                            if (len(bought_stock) > 1000):
                                                ...
                                                try:
                                                    try:
                                                        webhook = DiscordWebhook(username="Jihun Bot", avatar_url="https://cdn.discordapp.com/attachments/794207652602708019/794572711376453642/4460d42506dfee4b6f7796acc1c6d604.gif", url=get_logwebhk(message.guild.id))
                                                        eb = DiscordEmbed(title='제품 구매 로그', description='[웹 패널로 이동하기](http://jihun-vend.kro.kr:8080/)', color=0x00ff00)
                                                    ...
                                                    try:
                                                        webhook = DiscordWebhook(username="Jihun Bot", avatar_url="https://cdn.discordapp.com/attachments/794207652602708019/794572711376453642/4460d42506dfee4b6f7796acc1c6d604.gif", url=get_buylogwebhk(message.guild.id))
                                                        webhook.add_embed(DiscordEmbed(description="<@" + str(message.author.id) + ">" + "님, `" + product_name + "` 제품 `" + str(buy_amount) + "`개 구매 감사합니다! :tada:"))
                                                        webhook.execute()
                                                    except:
                                                        pass
                                                ...
                                                try:
                                                    try:
                                                        webhook = DiscordWebhook(username="Jihun Bot", avatar_url="https://cdn.discordapp.com/attachments/816621102561492992/845274772275068958/unknown.png", url=get_logwebhk(message.guild.id))
                                                        eb = DiscordEmbed(title='제품 구매 로그', description='[웹 패널로 이동하기](http://jihun-vend.kro.kr:8080/)', color=0x00ff00)
                                                        eb.add_embed_field(name='디스코드 닉네임', value=str(message.author), inline=False)
                                                        eb.add_embed_field(name='구매 제품', value=str(product_name), inline=False)
                                                    ...
                                                    try:
                                                        webhook = DiscordWebhook(username="Jihun Bot", avatar_url="https://cdn.discordapp.com/attachments/816621102561492992/845274772275068958/unknown.png", url=get_buylogwebhk(message.guild.id))
                                                        webhook.add_embed(DiscordEmbed(description="<@" + str(message.author.id) + ">" + "님, `" + product_name + "` 제품 `" + str(buy_amount) + "`개 구매 감사합니다! :tada:"))
                                                        webhook.execute()
                                                    except:
                                                        pass
